...
<script type="text/javascript">
	...
	$(function(){
			...
			$(".myModal2Update").click(function(){
				$("#myModalLabel2").html("资产类别修改");
					$(":input[name='assetTypeName']").val("");
					$(":input[name='assetTypeDescripti']").val("");
					$(":input[name='assetTypeName']").removeAttr("readonly");
					$(":input[name='assetTypeDescripti']").removeAttr("readonly");
					if($(this).attr("lang")==1){
							$("#myModalLabel2").html("资产类别");
							$(":input[name='assetTypeName']").attr("readonly","readonly");
							$(":input[name='assetTypeDescripti']").attr("readonly","readonly");
					...
					if($(this).attr("lang")!=2){
							//* /2.准备发送的异步请求
							var url = "${pageContext.request.contextPath}/fixedAsset/findByAssetTypeId.action?assetTypeId="+$(this).attr("title");
							//jquery实现  post   Ajax请求
							$.post(url, function(data) {
									$("#form").attr("action","${pageContext.request.contextPath}/fixedAsset/updateAssetType.action");
									$(":input[name='assetTypeId']").val(data.assetTypeId);
									$(":input[name='assetTypeName']").val(data.assetTypeName);
									$(":input[name='assetTypeDescripti']").val(data.assetTypeDescripti);
							}, "json");// 替代  eval() 
					}
			});
	});
		...
		<div class="container-fluid">
			...
			<div class="row-fluid">
				<div class="span12">
					...
					<!-- BEGIN 网页的标题和面包屑-->
					<h3 class="page-title">
						资产类别管理 <small>资产类别管理页</small>
					</h3>
					<ul class="breadcrumb">
						<li>	<a href="${pageContext.request.contextPath}/index.jsp">系统管理</a>  <i class="icon-angle-right"></i>
								<a href="${pageContext.request.contextPath}/fixedAsset/findAssetTypeByPage.action">资产类别管理</a> 
						</li>
					</ul>
					<!-- END 网页的标题和面包屑-->
				</div>
					...
					<div class="actions">
						<div class="btn-group">
							...
							<div id="sample_2_column_toggler" class="dropdown-menu hold-on-click dropdown-checkboxes pull-right">
								<label><input type="checkbox" checked data-column="0">资产类别ID</label>
					...
					<table class="table table-striped table-bordered table-hover table-full-width"
						...
						<c:forEach items="${pageBean}" var="obj">
								<tr>
									...
									<td>
									<a class="myModal2Update"  href="#myModal2"  data-toggle="modal"  lang="1" title="${obj.assetTypeId }">${obj.assetTypeName }</a>
									</td>
									<td>
									<fmt:formatDate value="${obj.assetTypeDate}"  pattern="yyyy-MM-dd"/>
									</td>
									<td>
									${obj.assetTypeDescripti }
									</td>
									<td>
										<!-- begin操作按钮 -->
											<div class="btn-group" style="margin-bottom: 0px !important;">
												...
												<li><a href="#myModal2" data-toggle="modal" class="role-edit btn mini purple myModal2Update" title="${obj.assetTypeId }"><i class="icon-edit"></i>编辑</a></li>
												<li><a href="#"  title="${obj.assetTypeId }" class="btn mini red user_del"><i class="icon-trash"></i>删除</a></li>
												<li><a href="#myModal2"  data-toggle="modal"  lang="1" title="${obj.assetTypeId }" class="btn mini blue myModal2Update"><i class="icon-check"></i>查看</a></li>
										</ul>
										</div>
									</td>
								</tr>
...
<script type="text/javascript">
	$(function(){
		...
		$('#submit-button').click(function(){
			var assetTypeNameEle = $("input[name='assetTypeName']");
			
			if($.trim(assetTypeNameEle.val())==""){
				swal("请填写!", "资产类别名称不能为空！", "error");
				return false;
			}
			return true;
		});
...
<!-- begin 消息提示框 -->
	<form action="${pageContext.request.contextPath}/fixedAsset/saveAssetType.action" id="form" method="post">
	...
	<div id="myModal2" class="modal hide  fade" style="width:600px;" tabindex="-1" role="dialog" aria-labelledby="myModalLabel2" aria-hidden="true">
		<div class="modal-header ">
			...
			<h3 id="myModalLabel2"  style="color:black;">资产类别添加</h3>
		</div>
		<div class="modal-body" >
			<div class="row-fluid">
				<div class="span10 ">
					<div class="control-group">
						<label class="control-label">资产类别名称：</label>
						<div class="controls">
							<input name="assetTypeName" type="text"
			...
			<div class="row-fluid">
				<div class="span10 ">
					<div class="control-group">
						<label class="control-label">资产类别描述：</label>
						<div class="controls">
							<input name="assetTypeDescripti" type="text"
			...
			$(".user_del").click(
											function() {
												...
												swal(
														...
														function() {
															window.location.href = "${pageContext.request.contextPath}/fixedAsset/deleteAssetType.action?assetTypeId="
																	+ cId;
														});
