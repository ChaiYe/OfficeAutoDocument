var Inbox = function() {
		...
		$
				.getJSON(
						...
						function(res) {
							...
							chtml += '<li><span>从(' + pi.startRow + ')条到('
									+ (pi.startRow * 1 + pi.pageSize * 1)
									+ ')条 共 (' + pi.recordCount
									+ ')条</span></li>';
							...
							for ( var int = 0; int < res.fs.length; int++) {
								...
								if (row.paramSysparamByImportant != null) {
									chtml += '<td class="inbox-small-cells"><i class="icon-star inbox-started" flowid="'
											+ row.id + '"></i> </td>';
								} else {
									chtml += '<td class="inbox-small-cells"><i class="icon-star" flowid="'
											+ row.id + '"></i> </td>';
								}
								chtml += '<td class="hidden-phone">'
										+ row.hrEmployeeBySenderId.name
										+ '</td>'
										+ '<td class="view-message"><a href="#" url="mail_flow!ajaxLoadOneById?id='
										+ row.id + '">'
										+ row.mailTitle.substring(0, 30)
										+ '</a></td>';
								...
								chtml += '<td style="text-align:right;">' + de
										+ '</td></tr>';
							}
							chtml += '</tbody></table>';
	...
	var loadMessage = function(url) {
		...
		$
            ...
            <div class="portlet-title"><div class="caption">\
              <i class="icon-reorder"></i>标题：'
									+ res.mailTitle
									+ '\
             </div>\
             <div class="actions">';
							if (res.hrEmployeeBySenderId.id != res.paramSysparamByReciveTypeId.id) {
								chtml += '<a href="mail_flow!del?id='
										+ res.id
										+ '" class="btn blue mini"><i class="icon-trash"></i>删除</a> \
                    <a href="mail_flow!delete?id='
										+ res.id
										+ '" class="btn mini purple"><i class="icon-trash"></i>彻底删除</a>';
							}
							chtml += '</div></div>\
           <div class="portlet-title"><div class="caption">\
            <i class="icon-time"></i>发送时间： '
									+ de
									+ '&nbsp;&nbsp;&nbsp;\
                        发件人：'
									+ res.hrEmployeeBySenderId.name
									+ '&nbsp;&nbsp;&nbsp;收件人：'
									+ res.hrEmployeeByReceiveId.name
									+ '</div>\
          </div> \
            <div class="portlet-body">\
              <font style="font-weight: bolder; font-size:15px; color:#4D82A3;">文本内容：</font>'
									+ res.mailContent
									+ '\
              <hr/>\
              ...
              <font style="font-weight: bolder; font-size:15px; color:#4D82A3;">附件内容('
									+ res.mailAttachments.length
									+ ')：</font><br>';
							var attac = res.mailAttachments;
							for ( var int = 0; int < attac.length; int++) {
								chtml += '<a class="btn mini green" href="'
										+ attac[int].fileUrl
										+ '" ><i class="icon-file"></i>'
										+ attac[int].fileName
										+ '</a>&nbsp;&nbsp;';
	...
	var initTags = function(input) {
		input.tag({
			...
			source : function(query, process) {
				return [ 'Bob Nilson <bob.nilson@metronic.com>',
						'Lisa Miller <lisa.miller@metronic.com>',
						'Test <test@domain.com>', 'Dino <dino@demo.com>',
	...
	/*
	 * var loadCompose = function () { var url = 'mail_add.jsp';
	 * 
	 * loading.show(); content.html('');
	 *  // load the form via ajax $.post(url, {}, function (res) { $('.inbox-nav >
	 * li.active').removeClass('active'); $('.inbox-header > h1').text('写邮件');
	 * $('.inbox-nav > li').eq(0).nextAll().hide(); $('.inbox-nav').append('<li class="compose-btn"><a
	 * class="btn" href="mail_flow.jsp" ><i class="icon-arrow-left"></i>返回</a><b></b></li>
	 * '); loading.hide(); content.html(res);
	 * 
	 * initTags($('[name="to"]')); initFileupload(); initWysihtml5();
	 * 
	 * $('.inbox-wysihtml5').focus(); App.fixContentHeight(); App.initUniform();
	 * }); };
	 */
	...
	/*
	 * var loadSearchResults = function () { var url =
	 * 'inbox_search_result.html';
	 * 
	 * loading.show(); content.html('');
	 * 
	 * $.post(url, {}, function (res) { $('.inbox-nav >
	 * li.active').removeClass('active'); $('.inbox-header >
	 * h1').text('Search');
	 * 
	 * loading.hide(); content.html(res); App.fixContentHeight();
	 * App.initUniform(); }); };
	 */
	...
	return {
		...
		init : function() {
			// user star
			$('.inbox-small-cells >.icon-star').live(
					'click',
					function() {
						var $cur = $(this);
						var url = "mail_flow!setStar?id=" + $cur.attr('flowid')
								+ "&v=" + new Date().valueOf();
			...
			var pageIndex = 1;// 页码
			// 上页
			$('.inbox-nav .pagination-left').live(
					'click',
					function() {
						if ((pageIndex * 1 - 1) == 0) {
							alert('已经是第一页');
						...
						var url = topurl.substring(0, i + 1)
			...
			// 下页
			$('.inbox-nav .pagination-right').live(
					'click',
					function() {
			...
			$('.inbox-nav > li.inbox > a').click(
					function() {
						var url = "../pages_mail/mail_flow!ajaxLoadNewMail?v="
								+ new Date().valueOf();
						topurl = url;
						topname = 'inbox';
						pageIndex = 1;
			...
			$('.inbox-nav > li.sent > a').click(
					function() {
						var url = "../pages_mail/mail_flow!ajaxLoadSendMail?v="
								+ new Date().valueOf();
						topurl = url;
						topname = 'send';
						pageIndex = 1;
			...
			$('.inbox-nav > li.draft > a').click(
					function() {
						var url = "../pages_mail/mail_flow!ajaxLoadAllMail?v="
								+ new Date().valueOf();
						topurl = url;
						topname = 'draft';
						pageIndex = 1;
			...
			$('.inbox-nav > li.star > a').click(
					function() {
						var url = "../pages_mail/mail_flow!ajaxLoadStarMail?v="
								+ new Date().valueOf();
						topurl = url;
						topname = 'star';
						pageIndex = 1;
			...
			$('.inbox-nav > li.trash > a')
					.click(
							function() {
								var url = "../pages_mail/mail_flow!ajaxLoadTrashMail?v="
										+ new Date().valueOf();
								topurl = url;
								topname = 'trash';
								pageIndex = 1;
			...
			// search mail
			$('#btn-searchmail').click(
					function() {
						var txt = $('#searchmail').val();
						url = "mail_flow!ajaxSearchMailByName?v="
								+ new Date().valueOf(), loadInbox('search',
								url, txt);
					});
			...
			if (App.getURLParameter("a") === "view") {
				...
				$.ajax({
					...
					cache : false,
					type : "get",
					url : "../pages_mail/mail_flow!ajaxLoadNewCount?v="
							+ new Date().valueOf(),
					dataType : "text",
					success : function(d) {
						if (d.trim() != "") {
							$('.inbox-nav > li.inbox > a').text(
									'新邮件(' + d + ')');
						}
					},
					error : function() {
