							...
							<table
								...
								<tbody>
									<c:forEach items="${objs}" var="obj">
										<tr>
											<td style="height: 3px;">${fn:substring(obj.sender,0,25)}${fn:length(obj.sender)>25
												? '...' : ''}</td>
											<td style="height: 3px;">${fn:substring(obj.subject,0,22)}${fn:length(obj.subject)>22
												? '...' : ''}</td>
											<td style="height: 3px;">${obj.time}</td>
											<td style="height: 3px;"><a
												class="search btn mini green"
												href="${pageContext.request.contextPath}/ParseMail.action?mailId=${obj.mailId}">
													<i class="icon-search"></i>查看详情 </a> <a href="#"
												title="${obj.mailId}" class="user_del search btn mini red">
									...
									.click(
											function() {
												...
												swal(
														...
														function() {
															window.location.href = "${pageContext.request.contextPath}/deleteMail.action?mailId="
																	+ cId;
														});
